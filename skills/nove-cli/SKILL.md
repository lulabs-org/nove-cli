---
name: nove
description: 使用 nove CLI 配置和访问 Nove API，查询或管理会议、参会人、会议记录、转写、参会者总结与用户。用户提到 nove、nove-cli、Nove 会议记录、会议转写、参会者总结、用户导入，或明确要求通过 Nove 命令行完成任务时使用；不用于创建飞书、腾讯会议等第三方日程。
---
