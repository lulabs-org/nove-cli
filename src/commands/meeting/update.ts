...
import { MEETING_TYPES, PROCESSING_STATUSES, validateDateRange } from '../../utils/validation.js';
...
export default class MeetingUpdate extends Command {
  ...
  static flags = {
    ...
    'processing-status': Flags.string({ aliases: ['status'], description: 'Processing status', options: [...PROCESSING_STATUSES] }),
  ...
  public async run(): Promise<void> {
    ...
    try {
      ...
      const body = Object.fromEntries(Object.entries({
        ...
        processingStatus: flags['processing-status'],
        title: flags.title,
        type: flags.type,
